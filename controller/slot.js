    ...
    axios
      ...
      .then((response) => {
        if (response.status == 200) {
          ...
          reject(
            new Error(
              `Maximum trying limit reached.. Please try after ${response.headers["Retry-After"]} minutes!`
            )
          );
        } else {
      ...
      .catch((error) => {
        reject(
          new Error(`Error fetching centres for pincode ${myPincode}: ${error}`)
        );
      });
  });
};
...
exports.getCentersByDistrict = (myDate, preference) => {
  ...
  console.warn(
    `Finding slots for ${myVaccine} for date: ${myDate} in ${preference.district.DISTRICT} district...`
  );
  var options = {
    params: {
    ...
    axios
      ...
      .then((response) => {
        if (response.status == 200) {
          ...
          reject(
            new Error(
              `Error fetching centres in district ${preference.district.DISTRICT}: ${response.statusText}`
            )
          );
        }
      })
      .catch((error) => {
        reject(
          new Error(
            `Error fetching centres in district ${preference.district.DISTRICT}: ${error}`
          )
        );
      });
  });
    ...
    axios
      ...
      .then((response) => {
        if (response.status == 200) {
          ...
          reject(
            new Error(`Error booking appointment: ${response.statusText}`)
          );
        }
      })
      .catch((error) => {
